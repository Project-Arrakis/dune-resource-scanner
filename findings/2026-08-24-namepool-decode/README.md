...
handle, and bounding geometry, with **no type identity of their own**. A real, named actor
(with a UClass, and therefore a name resolvable through this now-decoded pool) likely only
comes into existence when something — most plausibly player proximity, consistent with
§5c's finding that POI structure geometry is streamed per-player — promotes a slot into a
full object. Resource *actors* were separately confirmed resident map-wide (unlike POI
structures), which is presumably why positions are found reliably; but "resident" and
"already a named UObject" turn out to be different claims, and today's evidence points at
the record layer sitting on the wrong side of that line.
...
complete confirmed resource taxonomy, both reusable the moment any pointer *from* a record
*into* a nameable object is found — including, notably, if the "promote to actor on
proximity" theory is right, by simply visiting a predicted node and then immediately
re-scanning while still standing there, to see whether a resolvable actor (and hence an
FName reference into this pool) appears where none existed before.
